...
export function initIntro() {
  ...
  if (braceL) animate(braceL, { x: [-70, 0], opacity: [0, 1] }, { duration: 0.5, ease: EASE_OUT });
  if (braceR) animate(braceR, { x: [70, 0], opacity: [0, 1] }, { duration: 0.5, ease: EASE_OUT });
  if (word) {
    animate(
      word,
      { clipPath: ['inset(0 50% 0 50%)', 'inset(0 12% 0 12%)'], opacity: [0, 1] },
