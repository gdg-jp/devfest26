...
ダーク／ライトの切り替えはしません。代わりに `src/styles/tokens.css` の**テナントレイヤー**（`--t` / `--t-ht` / `--t-pa` / `--t-deep` / `--t-on`）だけを差し替えることで、他都市の DevFest に展開できます。ブランド定数（4色とパステル、Off White、Black 02）は共通で、テナントごとに変わりません。

`--t-deep`（明るい地の上で読めるアクセント色）と `--t-on`（`--t` のベタ面に乗せる文字色）を分けているのが要点で、黄色を基調にする九州テナントでは `--t-on` だけが黒に反転します。

切り替えは `src/data/site.ts` の `tenant` を変えるだけです。
