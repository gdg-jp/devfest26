...
**ビルドの結果は Studio から見えます。**「サイトに反映」の下に直近のビルドが出ます。失敗したときは**どの開催地のどのステップで落ちたか**と、**原因の抜粋**が並びます。抜粋は `scripts/annotate-failure.mjs` がビルドの出力の末尾を注釈として書き出したものです — Studio はトークンを持たないので**生のログは読めません**（未認証だと 403 です）。足りないときは GitHub のリンクを開いてください。
