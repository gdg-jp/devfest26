...
**リポジトリ変数 `GA_MEASUREMENT_ID` に GA4 の測定 ID（`G-` で始まるもの）を入れると計測が入ります。** シークレットではなく変数です — 測定 ID は公開された HTML に毎ページ出るもので、隠す対象ではありません。`SITE_URL` と同じ扱いです。

**未設定なら、タグは 1 バイトも出ません。** `preconnect` も `<script>` も出力されないので、ローカルのビルドと PR のビルドは何も設定しなくても数字に混ざりません。逆に、**設定されていて形が違う場合はビルドが落ちます**（`UA-…` や `GTM-…` を貼ってしまった場合）。黙って 1 か月何も取れていないより、赤いビルドのほうが安いためです（`src/lib/analytics.ts`）。
...
サイト全体で GA4 プロパティは 1 つです。都市と言語は URL にも出ていますが、レポートで束ねられるよう **event パラメータとしても送ります**。
...
**この 2 つは GA4 の管理画面で「カスタム定義」に登録しないとレポートに出ません。** 管理 → カスタム定義 → カスタムディメンション で、スコープを**イベント**にして 2 つ作ってください。送信自体は登録前から行われているので、登録した時点以降のデータが見えるようになります。
...
計測を足したい要素があれば、アンカーに `data-analytics="イベント名"` と（必要なら）`data-placement="…"` を書くだけです。拾っているのは `src/scripts/analytics.ts` の委譲リスナー 1 つで、それ以外に触る場所はありません。
