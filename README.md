...
**サイト全体のプロパティ 1 つと、開催地ごとのプロパティを併用します。** 1 ページから両方に送るので、チャプターは自分の数字だけを自分たちで見られて、サイト全体の合計と「トップページ → 開催地ページ」の導線も残ります。

|                            | 測定 ID の置き場所                                                   | 何が入るか                 |
| :------------------------- | :------------------------------------------------------------------- | :------------------------- |
| サイト全体（ロールアップ） | リポジトリ変数 `GA_MEASUREMENT_ID`                                   | 全開催地・全言語の全ページ |
| 開催地ごと                 | Studio の `event` ドキュメントの **Google Analytics Measurement ID** | その開催地のページだけ     |

**なぜ分けるのか。** GA4 には「1 つのプロパティの中で、この人には関西のデータだけ見せる」という権限設定がありません（UA の「ビュー」は廃止され、サブプロパティとロールアップは Analytics 360 の機能です）。関西の運営に関西の数字を渡す方法は、関西専用のプロパティを作ってそこに招待することだけです。

**どちらも任意です。** 開催地側が空でもロールアップには届き、ロールアップが未設定でも開催地側には届きます。チャプターは自分でプロパティを作って Studio に入れるだけで始められて、リポジトリの権限も他の都市の作業も要りません。

**両方とも未設定なら、タグは 1 バイトも出ません。** `preconnect` も `<script>` も出力されないので、ローカルのビルドと PR のビルドは何も設定しなくても数字に混ざりません。

**形が違う測定 ID はビルドを落とします**（`UA-…` や `GTM-…` を貼ってしまった場合）。`gtag` はその手の ID を黙って無視するので、気付くのは 1 か月後に数字を見に行った時です。Studio 側は入力欄でも弾きますが、すり抜けた場合は**その開催地のジョブだけ**が失敗し、その都市の公開済みページは据え置かれます。

⚠️ **サイト全体の測定 ID を開催地の欄に入れないでください。** 同じプロパティに二重に送られて数字が倍になります（同一 ID は重複除去しますが、意図しない設定であることに変わりはありません）。

**下書きプレビューは計測しません。** `preview/` は認証ゲートの向こうにあり、そこを見ているのは自分の下書きを確認している運営だけです。`.github/workflows/preview.yml` に `GA_MEASUREMENT_ID` が無いのはそのためです。

#### 送っているパラメータ

開催地と言語は URL にも出ていますが、レポートで束ねられるよう **event パラメータとしても送ります**。両方のプロパティに同じものを送るので、片方で作ったレポートがもう片方でもそのまま動きます。

| パラメータ      | 値                            | 名前の理由                                          |
| :-------------- | :---------------------------- | :-------------------------------------------------- |
| `devfest_city`  | `kansai` / `tokyo` / `portal` | GA4 の自動ディメンション `City`（地理）と別物なので |
| `site_language` | `ja` / `en`                   | 自動の `Language`（ブラウザの言語）と別物なので     |

**この 2 つは GA4 の管理画面で「カスタム定義」に登録しないとレポートに出ません。** 管理 → カスタム定義 → カスタムディメンション で、スコープを**イベント**にして 2 つ作ってください。**プロパティごとに必要です**（開催地を増やすたびに増えます）。送信自体は登録前から行われているので、登録した時点以降のデータが見えるようになります。

参加登録ボタンのクリックは `register_click` として送ります。`placement`（`topbar` / `hero` / `section`）が付くので、どの導線が効いたかが分かります。GA4 の拡張計測にも外部リンククリックはありますが、あれはフッターの connpass タグやパートナーのリンクと区別が付きません。キーイベント（旧コンバージョン）に指定しておくと扱いやすくなります。

計測を足したい要素があれば、アンカーに `data-analytics="イベント名"` と（必要なら）`data-placement="…"` を書くだけです。拾っているのは `src/scripts/analytics.ts` の委譲リスナー 1 つで、それ以外に触る場所はありません。イベントは設定済みの全プロパティに自動で送られるので、プロパティが増えてもこのファイルは変わりません。

#### プロパティ間で数字は一致しません

セッションはプロパティごとに数え直されるため、開催地のプロパティとロールアップでセッション数がずれます。ページビューはほぼ一致し、`_ga` クッキーが同一ドメインで共有されるのでユーザー数もおおむね揃います。運営間で数字を突き合わせる前に共有しておいてください。

データ保持期間や削除リクエストの設定も、プロパティの数だけ管理先が増えます。
...
- **publish** — `gh-pages` を取ってきて、**アーティファクトがある (言語, 都市) だけ**そのディレクトリを丸ごと差し替え、無いものには触りません。Sanity から消えた都市は両言語とも削除します。**このジョブが走るのは「サイトに反映」と手動実行のときだけです** — プルリクエストと `main` へのマージはビルドで止まります
...
speakers: # 省略可。トークを持つセッションではトーク側に登壇者名を書きます
  - speaker-slug # src/content/speakers/<slug>.md
talks: # 省略可。1枠に複数の発表を並べる場合はトークを並び順どおりに指定
  - talk-01
  - talk-02
...
セッションとトークは **`Session ∋ Talks`（Session が複数の Talk を保持する）** 関係です。Sanity Studio では Session ドキュメント内の `talks` 配列（ドラッグ＆ドロップで並び替え可能）で紐付けます。Markdown の場合は Session 側の `talks` にトークの ID を並び順どおりに列挙します。
...
トークを持つセッションでは、**登壇者はトーク側に書きます**（セッション側の `speakers` は省略できます）。逆に、セッションにも `speakers` が無く、`talks` も 1 本も無い場合は**ビルドが落ちます**。

トークファイル（または Sanity ドキュメント）があるときだけ `/talks/<slug>` が生成されます。トークを使わない都市では `/talks/` は 1 ページも作られません — セッションのページがそのまま発表のページだからです。
...
**ビルドの結果は Studio から見えます。**「サイトに反映」の下に直近のビルドが出ます（反映したぶんだけです。プルリクエストや `main` へのマージのビルドは `publish` が走らないので、サイトについて何も言っていません）。失敗したときは**どの開催地のどのステップで落ちたか**と、**原因の抜粋**が並びます。抜粋は `scripts/annotate-failure.mjs` がビルドの出力の末尾を注釈として書き出したものです — Studio はトークンを持たないので**生のログは読めません**（未認証だと 403 です）。足りないときは GitHub のリンクを開いてください。

Studio を通さず作り直したいときは Actions から `Build` を手動実行してください。`作り直す開催地` に `kansai` のように書けば同じことができ、空欄なら全部です。

**`main` へのマージはサイトを変えません。** 全開催地をビルドして、そこで止まります。目的はマージ結果がビルドできることの確認です — 別々に緑だったプルリクエスト 2 本が、合流して初めて落ちることはあります。**コードの変更がサイトに出るのは次の反映のとき**なので、すぐ出したいときは「サイトに反映」を押すか、`Build` を手動実行してください。
